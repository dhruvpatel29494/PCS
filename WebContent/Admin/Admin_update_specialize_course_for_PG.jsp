...
<title>Update Specialize Course for PG</title><link rel="shortcut icon" type="image/x-icon" href="<%=request.getContextPath() %>/Admin/images/logo_tab.png" />
	 	...
	 	<div class="page-content">
			...
			<div class="page-header">
				<div class="page-title">
					<h3>Update Specialize Course for PG</h3>
				</div>
			...
			<!-- Form validation -->
			<c:forEach items="${sessionScope.edit_specialize_course_for_PG }" var="x" >
            <form class="form-horizontal validate" action="<%=request.getContextPath() %>/Specialize_course_for_PG_Controller" method="post" >
	            <div class="block">
	                <h6 class="heading-hr"><i class="icon-checkmark-circle"></i> Form validation</h6>
						...
						<div class="col-sm-10">
                            <select name="PGfieldSpecificationId" class="required form-control">
                                ...
                                <c:forEach items="${sessionScope.search_PG_field_specification_Controller_for_specialize_course_for_PG }" var="i">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="required form-control" name="SpecializeCourseNameForPG" id="CountryName" value="${x.specialize_course_for_PG_name }">
						</div>
					</div>

					<div class="form-group">
						<label class="col-sm-2 control-label">Specialize Course Description for PG: <span class="mandatory">*</span></label>
						<div class="col-sm-10">
                            <textarea rows="5" cols="5" name="SpecializeCourseDescriptionForPG" class="required form-control">${x.specialize_course_for_PG_description }</textarea>
						</div>
					</div>
					
					<input type="hidden" name="SpecializeCourseIdForPG" value="${x.specialize_course_for_PG_id }"/>
					<input type="hidden" name="flag" value="update_specialize_course_for_PG"/>
